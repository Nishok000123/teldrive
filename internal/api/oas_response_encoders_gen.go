...
func encodeEventsEventsStreamResponse(response *EventsEventsStreamOKHeaders, w http.ResponseWriter) error {
	...
	// Encoding response headers.
	{
		...
		// Encode "Connection" header.
		{
			...
			if err := h.EncodeParam(cfg, func(e uri.Encoder) error {
				return e.EncodeValue(conv.StringToString(string(response.Connection)))
			}); err != nil {
...
func encodeUsersImportChannelResponse(response *ImportResult, w http.ResponseWriter) error {
	w.Header().Set("Content-Type", "application/json; charset=utf-8")
	w.WriteHeader(200)
