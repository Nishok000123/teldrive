...
var jsonFieldsNameOfChannel = [3]string{
	0: "channelName",
	1: "channelId",
	2: "selected",
}
...
// Encode implements json.Marshaler.
func (s *ChannelImport) Encode(e *jx.Encoder) {
	e.ObjStart()
	s.encodeFields(e)
	e.ObjEnd()
}
...
// encodeFields encodes fields.
func (s *ChannelImport) encodeFields(e *jx.Encoder) {
	{
		if s.Path.Set {
			e.FieldStart("path")
			s.Path.Encode(e)
...
var jsonFieldsNameOfChannelImport = [1]string{
	0: "path",
}

// Decode decodes ChannelImport from json.
func (s *ChannelImport) Decode(d *jx.Decoder) error {
	if s == nil {
		return errors.New("invalid: unable to decode ChannelImport to nil")
	}
	...
	if err := d.ObjBytes(func(d *jx.Decoder, k []byte) error {
		...
		return errors.Wrap(err, "decode ChannelImport")
	}
...
// MarshalJSON implements stdjson.Marshaler.
func (s *ChannelImport) MarshalJSON() ([]byte, error) {
	e := jx.Encoder{}
	s.Encode(&e)
	return e.Bytes(), nil
}
...
// UnmarshalJSON implements stdjson.Unmarshaler.
func (s *ChannelImport) UnmarshalJSON(data []byte) error {
	d := jx.DecodeBytes(data)
	return s.Decode(d)
}
...
var jsonFieldsNameOfFile = [14]string{
	0:  "id",
	1:  "uploadId",
	2:  "name",
	3:  "type",
	...
	for i, mask := range [1]uint8{
		...
		if result := (requiredBitSet[i] & mask) ^ mask; result != 0 {
			...
			for bitN := 0; bitN < missed; bitN++ {
				...
				fieldIdx := i*8 + bitIdx
				var name string
				if fieldIdx < len(jsonFieldsNameOfImportResult) {
					name = jsonFieldsNameOfImportResult[fieldIdx]
				} else {
					name = strconv.Itoa(fieldIdx)
				}
				failures = append(failures, validate.FieldError{
